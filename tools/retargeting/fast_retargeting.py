...
def retarget(
    ...
    src_skeleton = SkeletonInfo(src_skeleton_json)
    tgt_skeleton = SkeletonInfo(tgt_skeleton_json)
    ...
    if with_src_motion:
        for name, data in src_motion_data.items():
            if name != "transl":
                ...
                src_motion_data_renamed[bone_name] = data
        src_motion_data_renamed["transl"] = motion_data["transl"]
...
def main(
    ...
    for k,v in src_smpl_data.items():
        src_smpl_data[k]=v[0][None]
    ...
    print(time.time() - T)
    return motion_data, src_motion_data
