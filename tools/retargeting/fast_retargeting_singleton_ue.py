...
def retarget(
    ...
    src_name2bone: Optional[Dict[str, str]] = None,
    ...
    Returns:
        Tuple[Dict[str, List[List[float]]], Dict[str, List[List[float]]]]:
            motion_data & src_motion_data
    """
    with_src_motion = bool(src_name2bone)

    src_motion = SMPLMotion(src_smpl_data, insert_rest_pose=True)
    ...
    motion_data = {}
    src_motion_data = {}
    for frame in range(src_motion.n_frames):
        if frame % 5 == 0:
        ...
        for name in src_skeleton.bone_names:
            ...
            if tgt_bone.parent is None:
                tgt_local_matrix = tgt_bone.matrix_local
            else:
            ...
            # [*] Retargeting
            mat_basis_tgt = calculate_one_bone_retargeting(
                is_pelvis=is_pelvis,
                ...
                mat_0_src=mat_0_src,
                mat_0_tgt=mat_0_tgt,
                tgt_local_matrix=tgt_local_matrix,
                mat_src=mat_src,
            )
            matrix_basis_tgt_record[name] = mat_basis_tgt

            # [*] Record the result
            if frame > 0 or insert_src_rest_pose_at_0:
                T, Q, _ = mat_basis_tgt.decompose()
                rot = list(Q.to_euler())
                euler = np.rad2deg(rot).tolist()
                motion_data.setdefault(name, []).append(euler)
                if is_pelvis:
                    motion_data.setdefault("transl", []).append(list(T))

                if with_src_motion:
                    rot = src_motion.get_bone_rotation(name, frame)
                    euler = np.rad2deg(rot.as_euler('xyz')).tolist()  # type: ignore
                    src_motion_data.setdefault(name, []).append(euler)
                    if is_pelvis:
                        transl = src_motion.get_transl(frame)
                        src_motion_data.setdefault("transl", []).append(transl.tolist())

    motion_data_renamed = {}
    for name, data in motion_data.items():
        if name != "transl":
            bone_name = tgt_name2bone[name]
            motion_data_renamed[bone_name] = data[0]
    motion_data_renamed["transl"] = motion_data["transl"][0]

    src_motion_data_renamed = {}
    if with_src_motion:
        for name, data in src_motion_data.items():
            if name != "transl":
                bone_name = src_name2bone[name]
                src_motion_data_renamed[bone_name] = data[0]
        src_motion_data_renamed["transl"] = motion_data["transl"][0]
    return motion_data_renamed, src_motion_data_renamed
    ...
    for frame in range(2):
        ...
        for name in src_skeleton.bone_names:
            ...
            if frame > 0:
                ...
                transl = list(T)
                rot = list(Q.to_euler())
                euler = np.rad2deg(rot)
                # blender to ue
                transl = [-transl[0], transl[1], transl[2]]
                euler = (-euler).tolist()
                motion_data.setdefault(name, []).append(euler)
                if is_pelvis:
                    motion_data.setdefault("transl", transl)
...
def main(
    ...
    # motion_data, src_motion_data = retarget(
    #     src_smpl_data,
    #     tgt_name2bone=tgt_name2bone,
    #     src_name2bone=src_name2bone,
    #     src_skeleton_json=src_skeleton_json,
    #     tgt_skeleton_json=tgt_skeleton_json,
    #     insert_src_rest_pose_at_0=insert_src_rest_pose_at_0,
    # )
    ...
    print(f"🐞 cost time: {cost}s. {cost / 300} per loop")
...
if __name__ == '__main__':
    motion_data = main(
        src_smpl_path=Path("data/inference_result.npz"),
    )
    print(motion_data)
